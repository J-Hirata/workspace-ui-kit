...
`npm run build` が通る Next.js アプリのため、**ホスティング先で Node として起動する**のが基本です。リポジトリ内の `data/*.json` はビルド時に読み込まれるサンプル種データなので、そのまま「デモ用の本番ビルド」に使えます。

| 方式 | 内容 |
|------|------|
| **Vercel** | GitHub 等と連携して Import。`npm run build` がそのまま走り、課題提出用の **公開 URL を最短で**用意しやすいです。 |
| **自前サーバー** | サーバー上で `npm ci` → `npm run build` → `npm run start`（既定ポート 3000。`PORT` 環境変数で変更可）。 |
| **静的ホスティング（Surge 等）** | `next.config` に `output: 'export'` を足して `out/` を生成する方法があります。利用 API に [制約](https://nextjs.org/docs/app/guides/static-exports) があるため、採用する前に公式ドキュメントで確認してください。 |

静的 HTML だけを Surge に置く運用（別リポジトリの図解デモなど）とは前提が異なります。**まずは Vercel か `next start` のどちらか**を想定すると手戻りが少ないです。

### Pane 4「素材ライブラリ」

候補者ごとに **仮想フォルダ + ファイル** を Pane 4 下部に表示します。実体はブラウザの **IndexedDB**（`workspace-ui-kit-materials-v1`）に保存され、HTML 途中稿などをローカルに紐づけておく用途向けです。選考スコアカードの添付（JSON シード）とは別ストアです。共有・バックアップが必要になったら API + オブジェクトストレージへ差し替える前提の雛形です。
