...
export async function fetchAllTools(): Promise<Tool[]> {
  ...
  const detailRows = await sql`
    ...
    SELECT id, tool_id, text, created_at, sort_order, deletable
...
export async function upsertTool(tool: Tool): Promise<void> {
  ...
  for (let i = 0; i < parsed.projectDetails.length; i++) {
    ...
    await sql`
      ...
      INSERT INTO project_details (id, tool_id, text, created_at, sort_order, deletable)

      VALUES (${d.id}, ${id}, ${d.text}, ${d.createdAt}, ${i}, ${d.deletable ?? false})
